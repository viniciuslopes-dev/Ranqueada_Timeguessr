...
const db = vi.hoisted(() => ({ queries: [] as Array<{ query: string; values: unknown[] }>, resolver: (_query: string, _values: unknown[]): unknown[] => [] }))
...
const request = (body: Record<string, unknown>) => new Request('http://localhost/api', { method: 'POST', headers: { Authorization: `Bearer ${'x'.repeat(43)}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ roomId, ...body }) })
beforeEach(() => {
...
afterEach(() => { if (original === undefined) delete process.env.DATABASE_URL; else process.env.DATABASE_URL = original })

describe('proteções da competição na API', () => {
  it('recusa correção antiga sem motivo antes de gravar partida', async () => {
    const response = await handler(request({ action: 'add_match', title: 'Antiga', playedAt: '2020-01-01', scores: [{ playerId, score: 40000 }] }))
  ...
  it('recusa datas impossíveis e datas futuras', async () => {
    for (const playedAt of ['2026-02-30', '2999-01-01']) {
      const response = await handler(request({ action: 'add_match', title: 'Daily', playedAt, scores: [{ playerId, score: 40000 }] }))
  ...
  it('uma chave de perfil inválida não revoga o acesso da sala', async () => {
    const response = await handler(request({ action: 'progress_command', playerId, profileToken: 'invalid', command: { type: 'challenge', opponentId: playerId } }))
  ...
  it('recupera perfil somente conferindo hash da chave', async () => {
    ...
    db.resolver = (query, values) => query.startsWith('select i.player_id') && values[2] === createHash('sha256').update(profileToken).digest('hex') ? [{ player_id: playerId }] : base(query, values)
  ...
  it('arquiva jogador sem excluir placares ou troféus', async () => {
    const base = db.resolver
    db.resolver = (query, values) => query.startsWith('update players set archived') ? [{ id: playerId }] : base(query, values)
  ...
  it('excluir partida antiga exige motivo e grava auditoria com revisão', async () => {
    ...
    expect((await handler(request({ action: 'delete_match', matchId, correctionReason: 'Partida duplicada' }))).status).toBe(200)
    const audit = db.queries.find(q => q.query.startsWith('insert into competition_audit'))!
    expect(JSON.parse(audit.values[2] as string)).toMatchObject({ matchId, previousDate: '2020-01-01', reason: 'Partida duplicada' })
  ...
  it('repete leitura quando outra atualização vence a gravação por revisão', async () => {
    ...
    db.resolver = query => {
      if (query.startsWith('select id, name, invite_code')) return [{ id: roomId, name: 'Teste', invite_code: 'ABCDEF', created_at: '' }]
      if (query.startsWith('select revision, state')) { reads++; return [{ revision: reads, state: {} }] }
      if (query.startsWith('update room_progress')) { updates++; return updates === 1 ? [] : [{ revision: 3 }] }
      return []
    }
