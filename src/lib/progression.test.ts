...
import { achievementProgress, applyProgressCommand, challengeResult, decoratePlayers, emptyProgress, leagueToday, reconcileProgress, rulesFor, validDay, weekClosed, weeklyStandings } from './progression'

const makeRoom = (): RoomSnapshot => ({
  room: { id: 'room', name: 'Amigos', invite_code: 'ABCDEF', created_at: '' },
  players: ['a', 'b', 'c', 'd', 'e'].map(id => ({ id, room_id: 'room', nickname: id.toUpperCase(), color: '#ff7043', created_at: '' })),
  matches: [], scores: [], rounds: [], progress: emptyProgress(),
})
function match(room: RoomSnapshot, day: string, values: number[], id = `m${room.matches.length}`, gameNumber: number | null = null) {
  room.matches.push({ id, room_id: 'room', title: id, game_number: gameNumber, played_at: day, created_at: `${day}T12:00:00Z` })
  room.scores.push(...values.map((score, index) => ({ id: `${id}:${index}`, room_id: 'room', match_id: id, player_id: room.players[index].id, score, created_at: `${day}T12:00:00Z` })))
  return id
}
...
describe('campeonato semanal', () => {
  ...
  it('não dá pontos competitivos sem adversário nem prêmio com participação insuficiente', () => {
    const room = makeRoom(); match(room, '2026-09-14', [50000])
  ...
  it('desempata por vitórias antes de placar somado', () => {
    const room = makeRoom()
    match(room, '2026-09-14', [10000, 5000]); match(room, '2026-09-15', [10000, 5000]); match(room, '2026-09-16', [0, 50000])
  ...
  it('é idempotente e revisa prêmios após correção', () => {
    const room = makeRoom(); match(room, '2026-09-14', [40000, 30000]); match(room, '2026-09-15', [40000, 30000])
    ...
    room.scores.filter(s => s.player_id === 'b').forEach(s => { s.score = 50000 })
  ...
  it('distingue campeão anterior de líder provisório', () => {
    const room = makeRoom(); match(room, '2026-09-14', [40000, 30000]); match(room, '2026-09-15', [40000, 30000]); match(room, '2026-09-21', [1000, 50000])
    room.progress = reconcileProgress(room, tuesday)
...
describe('conquistas, personalização e rivalidades', () => {
  it('não trata empate como revanche nem vitória por margem estreita', () => {
    ...
    expect(reconcileProgress(room, tuesday).earned.filter(e => e.playerId === 'a').some(e => ['revenge', 'close'].includes(e.code))).toBe(false)
  })
  it('concede revanche e margem estreita com evidência do adversário', () => {
    const room = makeRoom()
    for (let i = 14; i < 17; i++) match(room, `2026-09-${i}`, [30000, 40000])
    match(room, '2026-09-17', [40001, 40000])
    expect(reconcileProgress(room, tuesday).earned.filter(e => e.playerId === 'a' && ['revenge', 'close'].includes(e.code))).toHaveLength(2)
  })
  it('exige cinco rodadas dos dois para reconhecer uma virada', () => {
    const room = makeRoom(); const id = match(room, '2026-09-14', [41000, 40000])
    for (const playerId of ['a', 'b']) for (let round = 1; round <= 5; round++) room.rounds.push({ id: `${playerId}${round}`, room_id: 'room', match_id: id, player_id: playerId, round_number: round, round_score: playerId === 'b' ? 8000 : round === 5 ? 10000 : 7750, year_error: 0, distance_km: 1, created_at: '' })
  ...
  it('não permite equipar recompensas bloqueadas', () => {
    const room = makeRoom()
    expect(() => applyProgressCommand(room, 'a', { type: 'profile', avatar: '🦊', frame: 'gold', title: '', featured: [] }, tuesday)).toThrow('desbloqueados')
    expect(() => applyProgressCommand(room, 'a', { type: 'profile', avatar: '🦊', frame: 'none', title: 'champion', featured: [] }, tuesday)).toThrow()
  })
  it('muda regras apenas para a semana seguinte', () => {
    const room = makeRoom()
    const state = applyProgressCommand(room, 'a', { type: 'settings', bestDays: 5, scoring: 'everyone', emblem: '🚀', playful: false }, tuesday)
  ...
  it('exige aceite do destinatário e não aceita partidas já lançadas', () => {
    const room = makeRoom(); match(room, '2026-09-22', [40000, 30000])
    room.progress = applyProgressCommand(room, 'a', { type: 'challenge', opponentId: 'b' }, tuesday)
    const id = room.progress.challenges[0].id
    expect(() => applyProgressCommand(room, 'a', { type: 'respond', challengeId: id, response: 'accept' }, tuesday)).toThrow()
    ...
    match(room, '2026-09-23', [41000, 40000]); match(room, '2026-09-24', [50000])
    expect(challengeResult(room, room.progress.challenges[0], '2026-09-24')).toMatchObject({ games: 1, wins: 1, finished: false })
  })
  it('confronto expirado sem jogos não produz vencedor', () => {
    const room = makeRoom()
    const result = challengeResult(room, { id: 'x', from: 'a', to: 'b', created: '', start: '2026-09-01', end: '2026-09-14', status: 'active' }, '2026-09-22')
